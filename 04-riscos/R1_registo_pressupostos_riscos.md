...
| R1-33 | A folha `Espaco` aplica ao resultado operacional a taxa de esforço fiscal da via 2 no cenário Base | A comparação entre espaços é operacional e a estrutura fiscal é a mesma nos dois | Distorce o payback em valor absoluto, não a comparação relativa entre as duas opções | BAIXA | Financeira | ABERTO |
| R1-34 | O varrimento da folha `Espaco` tem resolução de 400 consultas/ano e o do `BreakEven` de 10.000 € | Legibilidade das tabelas | O ponto de viragem indicado é o primeiro degrau acima do real. Se a decisão ficar perto do limite, refinar por interpolação | BAIXA | Financeira | ABERTO |
| R1-35 | A faturação é derivada da capacidade (`FONTE_FAT` = 2), não introduzida à mão | É verificável e obriga a projeção a caber no espaço | Se a cliente fornecer uma projeção de faturação em vez dos drivers, passar `FONTE_FAT` a 1 — e perde-se a verificação de plausibilidade | MÉDIA | GS | ABERTO |
| R1-40 | Na folha `Tesouraria`, a contribuição mensal para a Segurança Social é a anual de cruzeiro a dividir por doze | Simplificação | No primeiro ano a base é a declarada, não o lucro do próprio ano. O perfil mensal difere, o total anual aproxima-se | BAIXA | Fiscalidade | ABERTO |
| R1-42 | A vista a três anos assume custos fixos e preço constantes | Não há informação para outra coisa | Não é uma projeção financeira completa: falta inflação de custos, renovação de equipamento e evolução da tabela de preços. Serve para responder a «quando recupero o que meti» | MÉDIA | Financeira, quando os inputs chegarem | ABERTO |
| R1-43 | A rampa de ocupação do ano 1 (15% a subir até 45%) e as taxas dos anos 2 e 3 são valores de trabalho | Nenhuma. Arbitrários | Determinam a faturação, a tesouraria e o payback. São os números mais influentes de toda a folha `Tesouraria` e os que menos fundamento têm | ALTA | C, com apoio de GS | ABERTO |
...
| R1-37 | Prazo de prescrição das contribuições à Segurança Social | 5 anos | Determina a exposição retroativa da folha `RiscoLaboral`. Confirmar o prazo e o momento de contagem | MÉDIA | Fiscalidade | ABERTO |
| R1-38 | Coima estimada em 50% das contribuições da entidade, na folha `RiscoLaboral` | 50% | ESTIMATIVA GROSSEIRA. A moldura das contraordenações por falta de comunicação de admissão e de pagamento de contribuições tem de ser confirmada. Os juros de mora são igualmente aproximados | MÉDIA | Fiscalidade ou advogado | ABERTO |
...
| R1-36 | **A utilização dos gabinetes é tratada como consequência da procura, não como escolha.** A comparação 2 vs. 3 gabinetes só é válida a procura constante | Alta, se lida sem cuidado | Comparar as duas opções à mesma percentagem de utilização atribui automaticamente mais 50% de consultas à opção maior e faz o terceiro gabinete parecer sempre vantajoso. Foi o erro corrigido na construção da folha | A folha `Espaco` varre procura, não utilização. Manter assim, e dizê-lo no relatório | Financeira |
| **R1-39** | **A cliente tem zero experiência empresarial e vai comprometer-se com arrendamento plurianual, obras e integração de pessoas.** As consequências de a matemática não funcionar são dívida pessoal, não um relatório desatualizado | Certa — é uma condição, não uma eventualidade | Uma recomendação tecnicamente correta mas incompreensível para a destinatária não é uma recomendação. E uma recomendação apresentada sem o cenário de baixa em dinheiro transfere para ela um risco que ela não tem como avaliar | E1 passa a ter, obrigatoriamente: primer de linguagem no cap. 2, condições em doentes por dia no cap. 5, e cenário de baixa, passivo laboral e pontos de não retorno no cap. 7. Ver D2 | GS |
| **R1-41** | **Não existe plano de captação de doentes, e é o pressuposto silencioso de todo o modelo.** A rampa de ocupação é, na prática, um plano comercial disfarçado de célula de Excel | Alta | Se ninguém souber justificar aquelas taxas de ocupação, o modelo está a assumir resolvida a parte mais difícil do negócio. Toda a tesouraria, o payback e a decisão sobre o terceiro gabinete assentam nelas | Exigir que a cliente justifique a rampa com origem concreta de doentes: base atual, referenciação, subsistemas, localização. O plano comercial está fora do âmbito; assumi-lo em silêncio não está | GS |
...
| B — Modelação | 12 | 1 |
| C — Parâmetros normativos | 10 | 3 |
| D — Riscos | 13 | 3 críticos (R1-14, R1-39, R1-41) |
